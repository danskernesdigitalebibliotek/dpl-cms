...
/**
 * Migrating and combining material_grid paragraphs to new type.
 *
 * Material_grid_automatic and material_grid_link_automatic are deprecated,
 * and has instead been replaced by material_grid_automatic_v3, that supports
 * both input by link, advanced CQL and search filters.
 *
 * This migrate action finds all these old paragraph types, gets the data,
 * builds a new material_grid_automatic_v3, and sets it in the place of the
...
function dpl_paragraphs_deploy_migrate_material_grid_link(): string {
  $old_paragraph_types = ['material_grid_automatic', 'material_grid_link_automatic'];
  $new_paragraph_type = 'material_grid_automatic_v3';
  ...
  $paragraph_ids = \Drupal::entityQuery('paragraph')
    ->condition('type', $old_paragraph_types, 'IN')
  ...
  foreach ($paragraph_ids as $pid) {
    ...
    foreach ($parent_field->getValue() as $delta => $item) {
      ...
      if ($paragraph->hasField('field_cql_search')) {
        $search_values['cql'] = $paragraph->get('field_cql_search')->getString();
      }

      if ($paragraph->hasField('field_material_grid_link')) {
        $link = $paragraph->get('field_material_grid_link')->getString();

        $search_values['link'] = $link;
        $search_values['cql'] = _dpl_paragraphs_get_filter($link, 'advancedSearchCql');
      ...
      $new_paragraph = Paragraph::create([
        ...
        'field_material_search' => $search_values,
      ] + $values);
      $new_paragraph->save();
  ...
  $placeholders = [
    '@old_types' => implode(', ', $old_paragraph_types),
  ...
  \Drupal::logger('dpl_paragraphs')->info(
    "Paragraph migration of @old_types to @new_type completed. @count / @total migrated.",
    $placeholders
  );
